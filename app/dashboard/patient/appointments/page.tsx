...
// Extracted static past appointments to avoid unnecessary state
const pastAppointmentsData: PastAppointment[] = [
  {
    id: 'p1',
...
export default function PatientAppointmentsPage() {
  ...
  // Removed unused setters to fix 'no-unused-vars'
  ...
  // 1. Moved triggerToast & fetchAppointments to the top and wrapped in useCallback
  ...
  const fetchAppointments = useCallback(async () => {
    try {
      ...
      if (data.success) {
        // 4. Replaced 'any' with appropriate type interface
        const mappedData = data.data.map((app: {
          _id: string;
          doctorId?: { doctorName?: string; specialization?: string; consultationFee?: number };
          appointmentDate: string;
          appointmentTime: string;
          appointmentStatus: 'Confirmed' | 'Pending' | 'Completed';
          paymentStatus: 'Pending' | 'Paid';
  ...
  // 2. Added fetchAppointments to dependency array
  useEffect(() => {
    fetchAppointments();
  }, [fetchAppointments]);
                    ...
                    <tr key={app._id} className="hover:bg-primary/5 dark:hover:bg-white/5 transition-colors group">
                      <td className="p-md">
                        <div className="flex items-center gap-sm">
                          {/* 5. Replaced img with next/image */}
                          <Image 
              ...
              >
                ...
                <div className="flex-1 text-left">
                  ...
                  <div className="mt-md flex gap-sm">
                    <button 
                      onClick={() => triggerToast("Summary details downloaded successfully.", "success")}
                    ...
                    <button 
                      onClick={() => triggerToast("Prescription file ready for download.", "success")}
                      className="flex-1 py-2 bg-white/20 dark:bg-white/5 border border-white/10 dark:border-white/5 rounded-lg font-label-sm text-on-surface dark:text-slate-100 hover:bg-white/30 dark:hover:bg-white/10 transition-all flex items-center justify-center gap-xs cursor-pointer font-bold"
                    >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-outline-variant/20 dark:border-white/10 p-6 max-w-md w-full shadow-2xl space-y-4">
              <div className="flex justify-between items-center border-b border-outline-variant/10 pb-3">
                ...
                <button type="button" onClick={() => setShowScheduleModal(false)} className="text-on-surface-variant dark:text-slate-400 hover:opacity-80">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-outline-variant/20 dark:border-white/10 p-6 max-w-sm w-full shadow-2xl space-y-4">
              <div className="flex justify-between items-center border-b border-outline-variant/10 pb-3">
                ...
                <button type="button" onClick={() => setShowRescheduleModal(false)} className="text-on-surface-variant dark:text-slate-400">
                  <span className="material-symbols-outlined">close</span>
                </button>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-outline-variant/20 dark:border-white/10 p-6 max-w-sm w-full shadow-2xl space-y-4">
              <div className="flex justify-between items-center border-b border-outline-variant/10 pb-3">
                ...
                <button type="button" onClick={() => setShowViewModal(false)} className="text-on-surface-variant dark:text-slate-400">
                  <span className="material-symbols-outlined">close</span>
                </button>
              </div>
              <div className="space-y-3 text-left">
                <div className="flex items-center gap-sm">
                  {/* 5. Replaced img with next/image */}
                  <Image className="w-12 h-12 rounded-full object-cover" src={selectedAppointment.avatarUrl} alt="" width={48} height={48} unoptimized={true} />
                  <div>
                    <p className="font-label-md text-label-md text-on-surface dark:text-slate-100 font-bold">{selectedAppointment.doctorName}</p>
                    <p className="font-label-sm text-label-sm text-on-surface-variant dark:text-slate-400">{selectedAppointment.specialization}</p>
                  </div>
                </div>
                ...
                <div className="text-xs space-y-2 text-on-surface-variant dark:text-slate-300">
                  <p><span className="font-bold text-on-surface dark:text-slate-200">Date:</span> {selectedAppointment.appointmentDate}</p>
                  <p><span className="font-bold text-on-surface dark:text-slate-200">Time / Type:</span> {selectedAppointment.appointmentTime}</p>
                  <p><span className="font-bold text-on-surface dark:text-slate-200">Location:</span> {selectedAppointment.hospital}</p>
                  <p><span className="font-bold text-on-surface dark:text-slate-200">Status:</span> {selectedAppointment.appointmentStatus}</p>
                </div>
              </div>
